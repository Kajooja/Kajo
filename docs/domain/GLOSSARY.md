...
| Endorsement / yhteinen tykkäys | `Endorsement` | Actor-specific positive SharedProfile decision that an Item is worth doing together. Pending Endorsement is separate from Shared saved state; unanimity among currently accepted members promotes the Item to Shared saved/system `Tallennetut`. |
...
| List membership | `ItemListEntry` | Relation placing one Item in one ItemList. Stores adding actor and added time for traceability; does not copy rating/consumed state. |
