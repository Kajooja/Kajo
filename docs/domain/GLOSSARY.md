...
| Endorsement / yhteinen tykkäys | `Endorsement` | Actor-specific positive SharedProfile decision that an Item is worth doing together. In discovery the actor makes it by adding the Item to one custom List; there is no separate Like button. Pending Endorsement is separate from Shared saved state; unanimity among currently accepted members promotes the Item to Shared saved/system `Tallennetut`. |
| Pending endorsement | `PendingEndorsement` | Shared Item with at least one active Endorsement but without unanimous accepted-member consensus. It is hidden from endorsers' ordinary queue and prioritized for non-endorsing members. |
...
| Custom list | `CUSTOM` | User-named ItemList. Shared custom Lists are collaborative organization and do not require unanimous Endorsement. |
| List membership | `ItemListEntry` | Relation placing one Item in one ItemList. One picker action adds to exactly one List and preserves other existing memberships. It stores adding actor and added time for traceability and does not copy rating/consumed state. The discovery action may also emit positive evidence; the membership row itself remains organizational state. |
