...
## Complete-product planning handoff — 2026-09-07

Planning #213 preserves the owner's catalog, series, album, Helsinki events, friends/groups/review feeds, SleepLayer/compact-memory and distant network/local-learning/people-matching ideas in [FUTURE_PLAN.md](../product/FUTURE_PLAN.md). [ROADMAP.md](ROADMAP.md#post-mvp-execution-queue) appends packages 16–23 plus gated research after 14.0–15.3. The eventual target requires both Apple App Store and Google Play. All future IDs are unimplemented planning/research; no current MVP, device, database or promotion gate is closed by this documentation delivery.

Live GitHub inspection for this planning task confirmed PR #212 is merged into main at `d4a8246b7539e3e4f8c8d4bfe7e32b391924239e`; there were no open PRs at inspection. Earlier merge-approval statements below are historical checkpoints. Continue the remaining #208 schema-only capture/parity/replay prerequisite, then the algorithm sequence; do not try to merge #212 again. Planning changes use `docs/long-term-product-queue` from that main commit. No application/hosted changes or APK polling are part of this task.
...
Accepted implementation main is `d4a8246b7539e3e4f8c8d4bfe7e32b391924239e`: PR #212 is merged, including historical integrity/parity checks and proposed ADR-0006; #210 bootstrap backend/mobile refresh and #211 diagnostic are also on main. The unresolved follow-up is #208 schema-only capture and parity/replay proof. The owner requested bounded work with APK testing deferred.
...
**First algorithm coding task: [#207](https://github.com/Kajooja/Kajo/issues/207), roadmap 14.0, `MVP-ALG-001` + initial `MVP-ALG-009`.** Reproduce and fix bootstrap-only Personal ranking before more adaptive complexity. Issue #207 is open; its scoped core correction and isolated SQL/mobile tests are prepared in `fix/207-bootstrap-personal-ranking`, deployed to hosted Supabase; PR #210 is merged, while full acceptance remains pending. It relates to #185/#191 without reopening correctly completed implementation history merely for tracking.

**Pre-APK dependency (2026-09-07), PR #209:** merged. Settings now declares its Expo-compatible `expo-file-system` 57.0.5 dependency directly.

**New #207 prerequisite: [#208](https://github.com/Kajooja/Kajo/issues/208).** Public-source inspection found that the catalog migration’s literal single-line candidate-filter replacement does not match the earlier V0 function’s multiline body. Resolve reproducible database replay under the immutable-migration rule before claiming SQL CI acceptance. Do not skip failing history or silently edit deployed migrations. The #207 forward correction is deployed; isolated tests and the hosted smoke below do not complete full replay acceptance. APK grid/cache testing may proceed independently.

**Current bounded delivery:** forward migration `20260907155201_bootstrap_personal_ranking.sql` adds direct bootstrap LongTerm contribution using shared evidence selection/weight/decay semantics. New base version `prediction-v0.4-bootstrap` is recorded in base explanations and V1 traces. The complete V1 policy/trace function is preserved except for that version; no new dynamic string patch is introduced. `bootstrapServingVersion` / `bootstrapLongTerm` identify the contribution. Native score/confidence controls are unchanged; source-aware decay/support refinement remains 14.4.

Successful import commit/removal/calibration now invalidates mounted hosted rankings through a bounded session revision, including authorized Shared common-fit views. `npm run test:database` runs isolated PostgreSQL function fixtures and is included in `npm test`/CI. It explicitly does **not** claim full migration replay, full Supabase RLS/Auth, public V1 integration or device acceptance.

**Validation:** final `npm run check` passed (191 mobile, 14 catalog and 15 SQL runner tests, lint/typecheck, iOS/Android bundle exports). No phone/emulator interaction was performed. Hosted migration `20260907155201` was applied successfully. Its filename now matches the recorded deployment version; the applied SQL payload is unchanged. An admin transaction smoke passed authenticated public V1 ranking, positive bootstrap contribution, removal through the public import RPC, outsider denial and persisted base-model version. All synthetic data and traces were rolled back. Reusable smoke: `scripts/database/bootstrap-ranking.hosted-smoke.sql`. This bounded smoke is not complete end-to-end Auth/Shared/device acceptance.

**Next handoff:** #212 is merged; perform schema-only baseline capture and difference review under proposed ADR-0006. Keep #207/ALG-009 open until the remaining acceptance gates pass. Continue with roadmap 14.1 event/outbox/trace reliability after prerequisites. APK testing is deferred at the owner's request; **do not poll APK completion**. The Sprint 014 device checklist is retained for later.

**#208 reproducibility checkpoint:** `npm run diagnose:database-replay` now executes complete migration files unchanged in lexical timestamp order against fresh PGlite 0.3.14. It applies 33 files, then exits **1 / BLOCKED** at `20260904193000_catalog_provider_foundation.sql` with `P0001: Expected rank_items_v0 candidate filter was not found`. No failure is skipped or converted into success. Minimal Auth/role fixtures and a signature-only `rls_auto_enable` fixture are explicit; this does not emulate Supabase RLS/Auth or satisfy the pinned full-stack gate. Docker/Postgres executables are unavailable in this workspace.
...
At the earlier #212 checkpoint, CI #363 passed on an earlier head and automatic approval review rejected the merge pending explicit approval. That historical block is resolved: live GitHub inspection now confirms #212 merged at `d4a8246b7539e3e4f8c8d4bfe7e32b391924239e`. This does not complete schema-only baseline extraction or database acceptance. The earlier execution workspace lacked pg_dump/Docker and a direct database export connection. Next establish the pinned export environment and reconcile tracking/schema differences before any baseline deployment decision.
...
|---|---|
| Bootstrap taste missing from direct Personal base score | `rank_items_v0` reads native Events/interactions while `build_profile_memory_state_v1` includes bootstrap; a fresh Profile lacks historical Scenarios to bridge this. The pending #207 branch reproduces this in an isolated SQL fixture and adds a tested forward correction; main/hosted acceptance remains open. |
...
| DB verification | The pending #207 branch adds SQL function regression tests to npm test/CI. Full migration-reset replay is still blocked by #208; isolated fixtures do not satisfy it. Existing hosted migration files remain immutable. |
...
| Dependencies | Pre-APK branch declares the `expo-file-system` dependency used by `SettingsScreen.tsx` (merged in #209); Edge imports use floating `npm:@supabase/supabase-js@2`. Resolve with compatible explicit dependencies/pinning and verification in OPS-005, not an unrelated docs patch. Lockfile exists. |
...
The earlier documentation-only audit made no application or hosted changes; the subsequent bootstrap deployment is recorded above. Documentation link/requirement/diff checks are its local validation; code changes still require `npm run check` plus the new applicable DB gate.
...
Continue #208's remaining schema-only capture/parity/replay proof from accepted main, then execute the first unmet dependency in `ROADMAP.md`; the next coding task is specified above. Continue until a concrete gate is complete or a real external dependency blocks that task. Update this file with exact Issue/branch/PR, commit/deploy state, tests, blockers and one next action after each bounded slice. Never require the prior chat to reconstruct the plan. On completion of 15.3, activate the appended post-MVP queue; do not lose or silently implement the conditional/research backlog.
