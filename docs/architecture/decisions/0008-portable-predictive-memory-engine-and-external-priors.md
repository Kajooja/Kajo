...
Date: 2026-09-12  
Status: **design decision for review/acceptance through Issue #233 and its documentation PR; runtime implementation pending**
...
6. Start an isolated, reproducible MovieLens research pipeline according to [DATA_ENRICHMENT](../DATA_ENRICHMENT.md). Tag Genome enrichment and Beliefs analysis are optional later packets, not automatic training inputs or launch dependencies. Source permissions follow derived models and indexes.
...
`ROADMAP` owns packet order; `STATUS` owns the exact resumable task; `MVP` owns first-release acceptance. E1/D1/D2 establish executable contracts and an honest external-data report, not a requirement that a learned challenger must win. Production quality remains a separate gate. Optional D3/D4 and generations 2–5 are not silently promoted into MVP.
