...
The #228 List correction candidate extends the existing private resurfacing decision with active Profile-scoped List membership and entry age. Collection receipts invalidate mobile grid ranking identity even if interaction projection fields are unchanged. The candidate preserves function identity/ACLs, historical evidence and Shared consensus boundaries; hosted rollout remains pending.
