...
| Authentication | `apps/mobile/src/features/auth/` | Root-scoped persisted session state, email-or-nickname password sign-in, email+nickname registration, native confirmation deep-link parsing, scanner-safe recovery-code entry and in-app password update; unconfigured builds preserve the accepted mock flow |
| Password auth boundary | `supabase/functions/password-auth/` | One public Edge Function resolves email/nickname identifiers server-side, checks account existence, signs in, requests recovery and verifies recovery OTPs without returning resolved email or privileged keys |
