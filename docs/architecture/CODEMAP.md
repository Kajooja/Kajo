...
Sprints 001–008 are merged, validated and complete. Sprint 009 — Shared Kajo is active. The app now has one generic active Profile scope shared by PersonalProfile and ready SharedProfiles, a visible Shared Kajo setup/selection surface, and a shared Room identity slice in progress.
...
| Room feature | `apps/mobile/src/features/room/` | 2D Room home/navigation surface with bookshelf, projector and Shared Kajo wall object; active Profile identity and active Profile base theme are applied without changing navigation structure |
| Theme engine | `apps/mobile/src/theme/` | Personal Room tokens are preserved exactly; SharedProfiles resolve a stable restrained base identity deterministically from `profileId`; AmbientPhase remains a separate DiscoveryMode-driven overlay |
| Discovery feature | `apps/mobile/src/features/discovery/` | Global DiscoveryMode, generic grid/detail/swipe, hosted Prediction V0, rating/not-interested/save state, ordered persistence and exact undo; ranking/interaction scope and visible theme identity follow the active Profile |
