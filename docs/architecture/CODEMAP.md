...
The repository is in Sprint 001 Foundation. The first mobile engineering foundation now lives under `apps/mobile/` on Issue #2 / branch `feat/2-mobile-foundation` pending validation and merge.
...
|---|---|---|
| Mobile app | `apps/mobile/` | Foundation created; pending validation/merge |
...
| CI | `.github/workflows/ci.yml` | Created; requires dependency lockfile before `npm ci` can pass |
