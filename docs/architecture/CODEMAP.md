...
| Room feature | `apps/mobile/src/features/room/RoomScreen.tsx`, `apps/mobile/assets/room-cabin-2d.png` | Edge-to-edge minimalist front-facing 2D cabin with a muted palette, mobile-safe full bookshelf, animated DiscoveryMode atmosphere, independent breathing window/flickering fireplace Kajo layers and Home-only BOOK/MOVIE interaction targets. The shell retains the same scene beneath all authenticated routes; reduced-motion disables continuous Room motion; #149 keeps Home free of standalone title/helper/sign-out chrome |
