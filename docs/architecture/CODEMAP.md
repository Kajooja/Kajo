...
| List persistence | `supabase/migrations/20260901204135_profile_scoped_item_lists.sql` | Generic `item_lists`/`item_list_entries`, one system `Tallennetut` per Profile, safe Saved/consensus projection, explicit grants/RLS and security-invoker RPC boundary |
...
| Production DB security | `supabase/migrations/20260902061652_harden_production_function_boundaries.sql` | Private automatic-RLS trigger function, invoker-wrapped PersonalProfile lookup and deny-by-default privileges for future Data API objects |
