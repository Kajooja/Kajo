...
Sprints 001–010 are accepted. Sprint 011/#151 Shared discovery/Endorsement is accepted; #102 named Lists and Sprint 012/#138 messaging are hosted with refreshed configured Android acceptance deferred. Sprint 013/#156 Prediction nervous system is the active work. Sprint 013B hosted database verification is complete after one ordered forward fix; configured Android trace acceptance remains open.
...
| Prediction V1 nervous system | `public.rank_items_v1` + private memory/scenario helpers in `supabase/migrations/20260902223000_prediction_nervous_system_v1.sql`; hosted forward fix `supabase/migrations/20260904120420_fix_prediction_v1_candidate_returning.sql`; mobile call in `apps/mobile/src/features/discovery/predictionOperations.ts` | #156 first slice is hosted: authorized server-owned ranking, immutable run/candidate trace, deterministic working/short/long memory snapshot and inspectable same-Profile ScenarioMemory reranking. Hosted Personal/Shared authorization, trace integrity, outcome/undo isolation and representative query plans are rollback-tested. Configured-device trace acceptance remains open |
...
| DB migrations | `supabase/migrations/` | Identity/current state, Event persistence, Prediction V0/V1 evidence and ordered V1 forward correction, feedback/cooldown, Shared membership/invitations/lifecycle, Endorsement, Profile-scoped Lists and Profile messaging with explicit grants/RLS |
...
Do not create empty feature folders merely to match future architecture. Shared collaboration and Lists extend generic Profile/Item boundaries rather than creating `GroupTaste`, media-specific queues or a second recommender. Deployed migrations are not rewritten; hosted defects are corrected with ordered forward migrations.
