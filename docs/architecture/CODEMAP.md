...
Sprints 001–010 are accepted. Sprint 011/#151 Shared discovery/Endorsement is accepted; #102 named Lists and Sprint 012/#138 messaging are hosted with refreshed configured Android acceptance deferred. Sprint 013/#156 Prediction nervous system is the active work. Sprint 013B hosted + configured-Android Prediction V1 acceptance is complete. Sprint 013C now has a hosted immutable shadow/evaluation foundation; serving-aware manual promotion/rollback remains the next gate.
...
| Prediction V1 nervous system | `public.rank_items_v1` + private memory/scenario helpers in `supabase/migrations/20260902223000_prediction_nervous_system_v1.sql`; hosted forward fix `supabase/migrations/20260904120420_fix_prediction_v1_candidate_returning.sql`; mobile call in `apps/mobile/src/features/discovery/predictionOperations.ts` | Hosted and configured-device accepted: authorized server-owned ranking, immutable run/candidate trace, deterministic working/short/long memory snapshot, inspectable same-Profile ScenarioMemory reranking, Personal/Shared trace integrity and fallback distinction |
| SleepLayer V1 | `supabase/migrations/20260904170000_sleep_layer_v1_foundation.sql`, `supabase/migrations/20260904172000_sleep_layer_v1_fk_indexes.sql` | Hosted first 13C slice: immutable scalar PredictorGenomes, baseline Champion audit, three SHADOW Challengers, PredictionRun policy tagging, prospective job queue, complete frozen ShadowPrediction runs/candidates, mature exposed-outcome EvaluationWindows/GenomeEvaluations with Profile shrinkage, immutable assignment/promotion audit and private service-only worker/evaluator. Does not yet allow a Challenger to drive serving; manual serving promotion/rollback is the next gate |
...
| DB migrations | `supabase/migrations/` | Identity/current state, Event persistence, Prediction V0/V1 evidence, ScenarioMemory, SleepLayer shadow/evaluation foundation, feedback/cooldown, Shared membership/invitations/lifecycle, Endorsement, Profile-scoped Lists and Profile messaging with explicit grants/RLS |
...
Do not create empty feature folders merely to match future architecture. Shared collaboration, Lists and SleepLayer extend the generic Profile/Item/Event/Prediction architecture rather than creating media-specific queues or a second recommender. Deployed migrations are not rewritten; hosted defects/performance corrections use ordered forward migrations.
