...
| Authentication | `apps/mobile/src/features/auth/` | Root-scoped persisted session state, email-or-nickname password sign-in, email+nickname registration, native confirmation/recovery deep-link parsing and in-app password update; unconfigured builds preserve the accepted mock flow |
| Password auth boundary | `supabase/functions/password-auth/` | One public Edge Function resolves email/nickname identifiers server-side, checks account existence, signs in against Supabase Auth and requests recovery without returning resolved email or privileged keys |
| Auth email callback | `supabase/functions/auth-callback/` | Scanner-safe HTTPS hop forwards signup/recovery token hashes to the matching native Kajo route without consuming them; only the app verifies the token |
