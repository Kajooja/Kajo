...
| Authentication | `apps/mobile/src/features/auth/` | Root-scoped persisted login state, email-or-nickname password sign-in, email+nickname registration, redundant Android intent token parsing, explicit confirmation success, in-memory recovery sessions and return-to-login password updates; unconfigured builds preserve the accepted mock flow |
