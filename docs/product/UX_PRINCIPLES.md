...
12. **Committed actions should feel visible but calm.** Marking an Item read/watched should give restrained visual feedback, move the current card away and reveal the next Item without game-like effects. The committed Item remains available in consumed history.
13. **Recent choices are reversible.** Swipe/state actions should provide a clear undo/back-arrow affordance for recent decisions. Reversal must restore state predictably and must not create navigation or index jumps.
