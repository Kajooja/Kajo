...
- [-] `MVP-ENG-002` An isolated MovieLens manifest/adapter and baseline report are reproducible on a declared deterministic real-data cohort, with validation/quarantine, train-only artifacts, chronological and cold-start splits, a fixed final test, coverage/uncertainty/resource reporting and explicit task limitations. Include a bounded declared static-state versus ordered-prefix/trajectory-retrieval comparison. No external identities or invented context/exposures become native Kajo evidence. Dataset scale and experiment limits are reported honestly; a challenger is not required to win. D1 #236 / PR #242 now has an actual, independently reproduced 500-subject / 84,849-rating cohort from the owner-authorized 2018 GroupLens Kaggle v2 development source. Current-head source acceptance and D2 #237's bounded development report remain open; no shared benchmark or native uplift is claimed.
