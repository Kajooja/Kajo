...
- [-] `MVP-ENG-001` Executable generic engine contracts preserve Subject/acting-identity separation, Object/Action/State/Observation/Outcome semantics, missingness, time/version scope and hard constraints. Deterministic Kajo/media and small synthetic non-media adapter fixtures run without UI/hosted DB and without importing provider/UI/auth dependencies into core. The real package joins workspace/exports and root lint/typecheck/tests. Current SQL serving is unchanged until a parity-tested component replacement is admitted. E1 #235: source and fixtures implemented; published-head CI/acceptance remain separate.
