...
const ITEMS = [
  ...
  { id: 'consumed', itemType: 'MOVIE' as const, title: 'Consumed' },
...
describe('Shared discovery collaboration overlay', () => {
  it('prepends pending member choices and suppresses member history and actor endorsements', () => {
    expect(
      applySharedDiscoveryOverlay(ITEMS, 'MOVIE', STATE).map((item) => item.id),
    ).toEqual(['pending', 'ordinary']);
  });
