...
describe('mock discovery ranking', () => {
  it('returns the expanded requested generic Item type', () => {
    const books = getStaticMockItems('BOOK', 'FOR_YOU');
    const movies = getStaticMockItems('MOVIE', 'FOR_YOU');
  ...
  it('changes deterministic ordering by DiscoveryMode', () => {
    const forYou = getStaticMockItems('BOOK', 'FOR_YOU').map((item) => item.id);
    const surprise = getStaticMockItems('BOOK', 'SURPRISE').map((item) => item.id);
    const risk = getStaticMockItems('BOOK', 'RISK').map((item) => item.id);

    expect(forYou).not.toEqual(surprise);
    expect(surprise).not.toEqual(risk);
    expect(forYou).not.toEqual(risk);
    expect(getStaticMockItems('BOOK', 'FOR_YOU').map((item) => item.id)).toEqual(forYou);
  });
