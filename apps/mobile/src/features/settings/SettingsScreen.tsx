...
const IMPORT_SOURCES: ImportSource[] = [
  {
    key: 'letterboxd',
    label: 'Letterboxd',
    description: 'Valitse puretusta Letterboxd-exportista ratings.csv, watched.csv, diary.csv tai watchlist.csv.',
    kind: 'MOVIE',
    provider: 'LETTERBOXD',
  },
  {
    key: 'imdb',
    label: 'IMDb',
    description: 'Valitse IMDb:n oma ratings-, watchlist-, check-ins- tai list-CSV.',
    kind: 'MOVIE',
    provider: 'IMDB',
  },
  {
    key: 'books',
    label: 'Kirjahistoria',
    description: 'Goodreads- tai StoryGraph-CSV tunnistetaan automaattisesti. Myös geneerinen Kajo CSV toimii.',
    kind: 'BOOK',
  },
];
...
export function SettingsScreen() {
    ...
    () =>
      client
        ? async (functionName, arguments_) => {
            const { data, error: rpcError } = await client.rpc(functionName, arguments_);
            return {
              data,
  ...
  async function chooseAndStage(source: ImportSource) {
    ...
    try {
      ...
      if (file.size > 8 * 1024 * 1024) {
        setError('CSV-tiedosto on liian suuri tähän tuontiin. Enimmäiskoko on 8 Mt.');
        return;
      }
      if (file.extension.toLowerCase() === '.zip') {
        setError('Pura Letterboxd ZIP ensin ja valitse sen sisältä CSV-tiedosto.');
        return;
      }
  ...
  async function refreshCandidateItems(nextJob: HistoryImportJob) {
    ...
    setCandidateItems(Object.fromEntries(loaded.items.map((item) => [item.id, item])));
  }
  ...
  async function commit() {
    ...
    if (unresolved > 0) {
      setError('Korjaa tai ohita ensin kaikki epäselvät ja löytymättömät rivit.');
      return;
    }
    ...
    Alert.alert(
      ...
      [
        { text: 'Peruuta', style: 'cancel' },
        {
          text: 'Poista',
          style: 'destructive',
          onPress: () => void removeCurrentImport(),
        },
      ],
    );
  }

  async function removeCurrentImport() {
    if (!rpc || !job) return;
    setBusy(true);
    setError(null);
    const result = await removeHistoryImport(rpc, job.jobId);
    if (result.status === 'error') setError(result.message);
    else setJob(result.job);
    setBusy(false);
  }
  ...
  const unresolvedRows = job?.rows.filter(
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.bodyText}>
            Voit tuoda aiemmat katselu- ja lukutietosi omaan henkilökohtaiseen Kajoosi. Tuonti alustaa makuprofiiliasi, mutta uudet Kajo-valintasi painavat ajan myötä enemmän.
          </Text>
          <Text style={styles.bodyText}>
            Ryhmäprofiiliin ei tuoda historiaa suoraan. Myöhemmin ryhmän yhteinen Kajo voi käyttää hyväksyttyjen jäsenten henkilökohtaisia makusignaaleja common-fit-laskennassa ilman että henkilökohtainen historia kopioidaan ryhmään.
          </Text>
        </View>
        ...
        {!activeIsPersonal ? (
          <View style={styles.notice}>
            ...
            <Text style={styles.bodyText}>
              Olet nyt ryhmäprofiilissa. Vaihda henkilökohtaiseen Kajoosi, jotta voit lisätä tai muuttaa historiaa.
            </Text>
            <Pressable
          ...
          <>
            <View style={styles.section}>
              ...
              <Text style={styles.helpText}>
                Letterboxd: lataa omista asetuksistasi data-export, pura ZIP ja valitse haluamasi CSV. IMDb: vie ratings/watchlist/check-ins/list CSV. Kirjoissa Goodreads- ja StoryGraph-exportit tunnistetaan otsikoista.
              </Text>
            </View>
            ...
            {job ? (
              <View style={styles.section}>
                ...
                <Text style={styles.summaryTitle}>{job.fileName ?? job.sourceProvider}</Text>
                <View style={styles.statsRow}>
                  <Stat label="Rivejä" value={job.totalRows} styles={styles} />
                  <Stat label="Yhdistetty" value={job.matchedRows} styles={styles} />
                  <Stat label="Epäselvä" value={job.ambiguousRows} styles={styles} />
                  <Stat label="Ei löytynyt" value={job.unmatchedRows} styles={styles} />
                </View>

                {unresolvedRows.slice(0, 50).map((row) => (
                  <View key={row.rowId} style={styles.problemRow}>
                    <Text style={styles.problemTitle}>
                      {row.title}{row.releaseYear ? ` (${row.releaseYear})` : ''}
                    </Text>
                          ...
                          return (
                            ...
                            >
                              <Text style={styles.candidateText}>
                                {item?.title ?? 'Mahdollinen osuma'}{item?.releaseYear ? ` (${item.releaseYear})` : ''}
                              </Text>
                            </Pressable>
                          );
                        })}
                ...
                {unresolvedRows.length > 50 ? (
                  <Text style={styles.helpText}>
                    Näytetään ensimmäiset 50 tarkistettavaa riviä. Korjaukset päivittävät listaa.
                  </Text>
                ) : null}

                {job.status === 'COMMITTED' ? (
                  <View style={styles.successBox}>
                    <Text style={styles.successTitle}>Historia lisätty omaan Kajoon</Text>
                    <Text style={styles.bodyText}>
                      Yhdistetyt arviot sekä katsotut/luetut teokset vaikuttavat nyt pitkäaikaiseen makuprofiiliisi. Kajon omat uudet valinnat alkavat tämän jälkeen painaa enemmän.
                    </Text>
                    <Pressable
                      accessibilityRole="button"
                      disabled={busy}
                      ...
                      style={({ pressed }) => [styles.skipButton, pressed && styles.pressed]}
                    >
                      <Text style={styles.skipButtonText}>Poista tämän tuonnin vaikutus</Text>
                    </Pressable>
                  </View>
                ) : (
                  <Pressable
                    ...
                    style={({ pressed }) => [
                      ...
                      pressed && styles.pressed,
                      (busy || job.ambiguousRows + job.unmatchedRows > 0) && styles.disabled,
                    ]}
                  >
...
function createStyles(theme: ReturnType<typeof getRoomTheme>) {
  return StyleSheet.create({
    ...
    content: { paddingHorizontal: 20, paddingTop: 14, paddingBottom: 42, gap: 18 },
    title: { color: theme.base.textPrimary, fontSize: 26, fontWeight: '800' },
    section: { gap: 12, padding: 16, borderRadius: 16, borderWidth: 1, borderColor: theme.base.border, backgroundColor: theme.surface.panel },
    ...
    notice: { gap: 12, padding: 16, borderRadius: 16, borderWidth: 1, borderColor: theme.base.border, backgroundColor: theme.surface.panel },
    noticeTitle: { color: theme.base.textPrimary, fontSize: 16, fontWeight: '800' },
    importRow: { flexDirection: 'row', alignItems: 'center', gap: 12 },
    importCopy: { flex: 1, gap: 3 },
    importTitle: { color: theme.base.textPrimary, fontSize: 14, fontWeight: '700' },
    primaryButton: { minHeight: 44, borderRadius: 13, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 16, backgroundColor: theme.base.structureLight },
    primaryButtonText: { color: theme.base.appBackground, fontSize: 13, fontWeight: '800' },
    secondaryButton: { minHeight: 40, borderRadius: 12, borderWidth: 1, borderColor: theme.base.border, paddingHorizontal: 12, alignItems: 'center', justifyContent: 'center' },
    secondaryButtonText: { color: theme.base.textPrimary, fontSize: 12, fontWeight: '700' },
    summaryTitle: { color: theme.base.textPrimary, fontSize: 14, fontWeight: '700' },
    statsRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
    stat: { minWidth: 70, flexGrow: 1, padding: 9, borderRadius: 11, borderWidth: 1, borderColor: theme.base.border },
    statValue: { color: theme.base.textPrimary, fontSize: 18, fontWeight: '800' },
    statLabel: { color: theme.base.textMuted, fontSize: 10, marginTop: 2 },
    problemRow: { gap: 8, paddingTop: 12, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: theme.base.border },
    problemTitle: { color: theme.base.textPrimary, fontSize: 13, fontWeight: '700' },
    candidateList: { gap: 6 },
    candidateButton: { minHeight: 38, justifyContent: 'center', borderRadius: 10, borderWidth: 1, borderColor: theme.base.border, paddingHorizontal: 10 },
    candidateText: { color: theme.base.textPrimary, fontSize: 12, fontWeight: '600' },
    skipButton: { alignSelf: 'flex-start', minHeight: 34, justifyContent: 'center', paddingHorizontal: 4 },
    skipButtonText: { color: theme.base.textMuted, fontSize: 12, textDecorationLine: 'underline' },
    successBox: { gap: 8, paddingTop: 8 },
    successTitle: { color: theme.base.textPrimary, fontSize: 15, fontWeight: '800' },
