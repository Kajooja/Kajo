...
test('artificial ZIP passes the actual Python intake and built TypeScript adapter, with verified replay', async () => {
  ...
  try {
    const script = 'import sys; from pathlib import Path; sys.path.insert(0,"scripts/research"); '
      ...
      + 'root=Path(sys.argv[1]); archive,plan=fixture(root); path,_,_=normalize_archive(archive,root/"normalized",plan); print(path)';
